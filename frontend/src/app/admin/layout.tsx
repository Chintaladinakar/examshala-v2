            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.path}
                  className={`flex items-center gap-3.5 px-4 py-3 rounded-xl font-medium text-sm transition-all duration-200 ${isActive ? 'bg-teal-900 text-white shadow-inner font-semibold border-l-4 border-teal-400' : 'text-teal-100/80 hover:bg-teal-900/40 hover:text-white'}`}
                >
                  {item.icon}
                  <span>{item.name}</span>
                </Link>
